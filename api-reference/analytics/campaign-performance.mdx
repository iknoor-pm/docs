...
| `positive_replied` | `COUNT(DISTINCT lead)` tagged **positive** (`sentiment_type = 'positive'`), attributed to send date, no reply-event requirement. Same definition as the campaign analytics page and the [overall-stats tile](/api-reference/analytics/overview). |
