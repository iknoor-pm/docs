...
api: "GET https://server.smartlead.ai/api/v1/analytics/mailbox/name-wise-health-metrics"
description: "Get detailed health metrics by individual email account address"
---
...
<ParamField query="is_bounced" type="string">
  Set to "true" or "false" to filter by bounce status
</ParamField>

<RequestExample>

```bash cURL
curl "https://server.smartlead.ai/api/v1/analytics/mailbox/name-wise-health-metrics?api_key=YOUR_KEY"
```
...
response = requests.get(
    "https://server.smartlead.ai/api/v1/analytics/mailbox/name-wise-health-metrics",
...
const response = await fetch(
  `https://server.smartlead.ai/api/v1/analytics/mailbox/name-wise-health-metrics?api_key=${API_KEY}`
);
