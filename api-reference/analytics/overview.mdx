...
<Note>
  The rules below (inclusive date boundaries, send-date attribution, unique vs raw counts, and the rate formulas) are shared across every Global Analytics endpoint. See [How Analytics Metrics Are Calculated](/api-reference/analytics/how-metrics-are-calculated) for the full explanation.
</Note>
...
<ResponseField name="positive_replied" type="number">
  Count of **distinct leads tagged as positive** (`sentiment_type = 'positive'`), attributed to the **send date** of their email — no reply-event requirement. This uses the same definition as the single-campaign analytics page, so the tile, the day-wise positive chart, and the campaign page all reconcile. A positive reply that arrives later is counted on the day its email was sent.
</ResponseField>
