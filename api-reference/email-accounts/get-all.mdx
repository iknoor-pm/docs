...
api: "GET https://server.smartlead.ai/api/v1/email-accounts/"
description: "Retrieve all email accounts with advanced filtering and pagination options"
---
...
<Info>
The `tags` array is always included in the response. The `campaign_ids` array is only included when `fetch_campaigns=true` is passed.
</Info>
